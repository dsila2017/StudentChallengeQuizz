...
struct AnxietyResult: View {
    @State var model: QuestionViewModel
    var body: some View {
        VStack(spacing: 20) {
            Text(model.checkResult(type: model.anxietyQuestions))
                .font(.title.bold())
            Text("Your score is: \(model.checkScore(type: model.anxietyQuestions))")
                .font(.title2.bold())
...
#Preview {
    AnxietyResult(model: QuestionViewModel())
}
